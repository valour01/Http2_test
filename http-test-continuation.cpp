...
clock_t begTime;
clock_t timespend;
time_t begin_continuation;
time_t end_continuation;
struct tm * timeinfo;
string continuation_fatal;
...
static int submit_request(struct Connection *connection, struct Request *req, const nghttp2_priority_spec *pri_spec) {
    ...
    const nghttp2_nv nva[] = {MAKE_NV(":method", "GET"),
        ...
        MAKE_NV("accept", "*/*"),
        MAKE_NV("user-agent", "aaaaaaaaaaaaaaaaaaaaanghttp2/" NGHTTP2_VERSION)};

        time ( &begin_continuation);
        localtime ( &begin_continuation );
        cout<<"begin time:"<<begin_continuation<<endl;
...
struct Request req;
/*
 * Fetches the resource denoted by |ur
 */
static void basic_test(const struct URI *uri) {
    ...
    while (nghttp2_session_want_read(connection.session) ||
        ...
        int nfds = poll(pollfds, npollfds, 1000);//这里阻塞了
...
void print_result()
{
    ...
    time ( &end_continuation);
    localtime(&end_continuation);
    cout<<"end time:"<<end_continuation<<endl;
    cout<<"continuation count:"<<end_continuation - begin_continuation<<endl;
    continuation=(int)(end_continuation - begin_continuation);
    cout<<"int continuation:"<<continuation<<endl;
    cout<<"recv_data_with_con:"<<recv_data_with_con<<endl;
    insert_db();
}
...
void insert_db(){
          ...
          "http2_new", 0, NULL, 0) == NULL) 
  {
    ...
    string receive_headers_order;
    for(int i =0;i<receive_headers_frames.size();i++){
          receive_headers_order+=int_to_string(receive_headers_frames.at(i).stream_id);
          receive_headers_order+=":";
          receive_headers_order+=int_to_string(receive_headers_frames.at(i).size);
          receive_headers_order+="|";
    }

    string receive_data_order;
    for(int i =0;i<receive_data_frames.size();i++){
          receive_data_order+=int_to_string(receive_data_frames.at(i).stream_id);
          receive_data_order+=":";
          receive_data_order+=int_to_string(receive_data_frames.at(i).size);
          if(receive_data_frames.at(i).end==1)
          {
            receive_data_order+="E";
          }
          receive_data_order+="|";
    }

    string first_recv_sequence;
    for(int i=0;i<recv_sequence.size();i++){
        first_recv_sequence+=int_to_string(recv_sequence.at(i));
        first_recv_sequence+="|";
    }
    string finish_recv_sequence;
    for(int i =0;i<finish_sequence.size();i++){
        finish_recv_sequence+=int_to_string(finish_sequence.at(i));
        finish_recv_sequence+="|";
    }

    char statement[30000];
    continuation_fatal="fatal:"+fatal+"|recv_go_away:"+recv_go_away;
    if(totalrows==0){    
    snprintf(statement, 30000, "INSERT INTO result (uri, \
        support_ssl, \
        support_http2, \
        support_server_push, \
        server_push_disable_made, \
        rpp_after_spd, \
        receive_headers_order, \
        receive_data_order, \
        recv_sequence, \
        ...
        server, \
        fatal \
        ) VALUES ('%s','%d','%d','%d','%d','%d','%s','%s','%s','%s','%d','%s','%s','%s');", \
        test_uri, \
        support_ssl, \
        support_http2, \
        support_server_push, \
        server_push_disable_test_made, \
        server_push_disable_test_receive_push_promise, \
        receive_headers_order.c_str(), \
        receive_data_order.c_str(), \
        first_recv_sequence.c_str(), \
        finish_recv_sequence.c_str(), \
        support_range, \
        recv_go_away.c_str(), \
        server.c_str(), \
        fatal.c_str() \
        ...
        snprintf(statement, 30000, "UPDATE result set continuation='%d', \
        recv_data_with_con='%d', \
        continuation_fatal='%s' \
        where uri='%s';", \
        continuation, \
        recv_data_with_con, \
        continuation_fatal.c_str(), \
        test_uri);
    }
