    ...
    switch (frame->hd.type) {
        ...
        case NGHTTP2_HEADERS:
            if (nghttp2_session_get_stream_user_data(session, frame->hd.stream_id)) {
                ...
                if (frame->hd.stream_id == 7){
                    nghttp2_priority_spec temp_spec;
                    nghttp2_priority_spec_init(&temp_spec, 7, 1, 1);
                    nghttp2_submit_priority(session, NGHTTP2_FLAG_NONE, 1, &temp_spec);
                }
            }
            break;
...
static int submit_request(struct Connection *connection, struct Request *req, const nghttp2_priority_spec *pri_spec) {
    ...
    const nghttp2_nv nva[] = {MAKE_NV(":method", "GET"),
        ...
        MAKE_NV("accept", "*/*"),
       // MAKE_NV("RANGE", "bytes=100-300"),
        MAKE_NV("user-agent", "nghttp2/" NGHTTP2_VERSION)};
    
        const nghttp2_nv nva_range[] = {MAKE_NV(":method", "GET"),
            MAKE_NV_CS(":path", req->path),
            MAKE_NV(":scheme", "https"),
            MAKE_NV_CS(":authority", req->hostport),
            MAKE_NV("accept", "*/*"),
    ...
    if(pri_spec->weight==33){
        stream_id = nghttp2_submit_request(connection->session, pri_spec, nva_range,
                                       sizeof(nva) / sizeof(nva[0]), NULL, req);
    range_stream_id =stream_id;
    }
    else{
...
static void basic_test(const struct URI *uri) {
    ...
    nghttp2_priority_spec pri_spec7;
    ...
    nghttp2_priority_spec_init(&pri_spec7,0,33,0);
    int stream_id_7= submit_request(&connection, &req7, &pri_spec7);    
...
void insert_db(){
          ...
          "http2_new", 0, NULL, 0) == NULL) 
  {
    ...
    snprintf(statement, 30000, "INSERT INTO result (uri, \
        ...
        snprintf(statement, 30000, "UPDATE result set support_ssl = '%d', \
        support_http2='%d', \
        support_server_push='%d', \
        server_push_disable_made='%d', \
        rpp_after_spd='%d', \
        receive_headers_order='%s', \
        receive_data_order='%s', \
        recv_sequence='%s', \
        finish_sequence='%s', \
        support_range='%d', \
        receive_go_away='%s', \
...
int main(int argc, char **argv) {
    ...
    //memset(&my_result, 0, sizeof(struct test_result));
    
    int rv;
    
    //char *inputURI = (char*)malloc(1024 * sizeof(char));
    //scanf("%s", inputURI);
    //inputURI[1023] = '\0';
    //result_out<<inputURI<<endl;
